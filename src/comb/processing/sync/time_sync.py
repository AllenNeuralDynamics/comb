...
def calculate_monitor_delay_visual_coding(photodiode_rise, stim_vsync_fall):
    ''' Calculating monitor delay for visual coding stimuli
    TODO: merge and make it work with (def calculate_monitor_delay)
    '''
    ...
    if ptd_start is not None and ptd_end is not None:
        ...
        if (delay_std > DELAY_THRESHOLD or np.isnan(delay)):
            ...
            print("Sync photodiode error needs to be fixed. Using assumed monitor delay: {}".format(round(delay, ROUND_PRECISION)))
            raise
...
class OphysTimeAligner(object):
    def __init__(self, sync_file, scanner=None, dff_file=None,
        ...
        self._dataset = SyncDataset(sync_file)
