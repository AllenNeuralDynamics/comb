...
#     elif 'foraging' in data['items']:
#         stimuli = data['stimuli'][0] # always a single element list in passive?
...
def get_visual_stimuli_df(
    ...
    if data['items'][behavior_key]['params']['stage'] == 'STAGE_0':
    ...
    elif data['items'][behavior_key]['params']['stage'] == 'STAGE_1':
            ...
            for seg in range(len(display_sequence) - 1):
                for index, row in timing_table.iterrows():
                    if row.start >= display_sequence[seg, 1]:
                        ...
                        timing_table.loc[index, 'start'] = timing_table.start[index] - display_sequence[seg, 1]+ display_sequence[seg + 1, 0]
        ...
        try:
            omitted_flash_frame_log = \
                data['items'][behavior_key]['omitted_flash_frame_log']
        except KeyError:
            # For sessions for which there were no omitted flashes
            omitted_flash_frame_log = dict()
